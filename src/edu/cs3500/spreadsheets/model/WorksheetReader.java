...
public final class WorksheetReader {
  ...
  public interface WorksheetBuilder<T> {
    ...
    WorksheetBuilder<T> setRowSize(int i, int size);

    WorksheetBuilder<T> setColSize(int i, int size);
  }
  ...
  public static <T> T read(WorksheetBuilder<T> builder, Readable readable) {
    ...
    while (scan.hasNext()) {
      ...
      } else if (cell.startsWith("rowsize:")) {
        cell = cell.substring(8);
        String[] rowsize = cell.split(";");
        for (String s : rowsize) {
          String[] split = s.split(",");
          builder = builder.setRowSize(Integer.parseInt(split[0]),
                  Integer.parseInt(split[1]));
        }

      } else if (cell.startsWith("colsize:")) {
        cell = cell.substring(8);
        String[] colsize = cell.split(";");
        for (String s : colsize) {
          String[] split = s.split(",");
          builder = builder.setColSize(Integer.parseInt(split[0]),
                  Integer.parseInt(split[1]));
        }
